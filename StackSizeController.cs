...
namespace Oxide.Plugins
{
    ...
    class StackSizeController : CovalencePlugin
    {
        ...
        private bool _ssctcSplittingGuard = false; // reentrancy guard for splitting

        private readonly HashSet<ItemId> _fromCupboard = new HashSet<ItemId>(); // track items taken from TC
        ...
        private class Configuration
        {
            ...
            // --- Cupboard-only settings ---
            public bool EnableCupboardContainerOverride = true;
            public float CupboardContainerMultiplier = 2.0f;
        }
...
int baseSize = GetStackSize(item.info);
try
{
    var container = item.parent ?? item.GetRootContainer();
    if (container != null && container.HasFlag(ItemContainer.Flag.IsPlayer))
    {
        // Allow a giant stack only if it's truly oversized and NOT coming from TC
        if (!_fromCupboard.Contains(item.uid) && item.amount > baseSize)
                    {
                        return Mathf.Clamp(item.amount, 1, int.MaxValue);
                    }
    }
}
catch { }
try
            {
                if (_config.EnableCupboardContainerOverride && _config.CupboardContainerMultiplier > 0f)
                {
                    if (IsInToolCupboard(item))
                    {
                        return Mathf.Clamp(Mathf.RoundToInt(baseSize * _config.CupboardContainerMultiplier), 1, int.MaxValue);
                    }
                }
            }
            catch { /* swallow */ }
        ...
        private bool IsInToolCupboard(Item item)
        {
            try
            {
                var container = item?.parent ?? item?.GetRootContainer();
                ...
                try { owner = container.entityOwner; } catch { }
                if (owner == null) return false;

                // Prefer type check when available
                bool isCupboard = false;
                try { isCupboard = owner is BuildingPrivlidge; } catch { }

                // Fallback checks
                if (!isCupboard)
                {
                    try { isCupboard = owner.ShortPrefabName == "cupboard.tool.deployed"; } catch { }
                }
                if (!isCupboard)
                {
                    try
                    {
                        var nm = owner.name ?? string.Empty;
                        if (!string.IsNullOrEmpty(nm))
                            isCupboard = nm.IndexOf("cupboard.tool", System.StringComparison.OrdinalIgnoreCase) >= 0;
                    }
                    catch { }
                }
                return isCupboard;
        ...
        private bool IsCupboardContainer(ItemContainer container)
        {
            if (container == null) return false;
            BaseEntity owner = null;
            try { owner = container.entityOwner; } catch { }
        ...
        private void OnItemRemovedFromContainer(ItemContainer container, Item item)
        {
            try
            {
                ...
                if (IsCupboardContainer(container))
                {
                    _fromCupboard.Add(item.uid); // temporary marker (case-insensitive 'add' will fix below if needed)
                    timer.Once(2f, () => _fromCupboard.Remove(item.uid));
                }
            }
            catch { }
        }
...
private void OnItemAddedToContainer(ItemContainer container, Item item)
        {
            try
            {
                ...
                if (_ssctcSplittingGuard) return;

                // If destination is a cupboard, do nothing (cupboard can have higher limit)
                if (IsCupboardContainer(container)) return;

                // Only split when destination is player inventory
                if (!container.HasFlag(ItemContainer.Flag.IsPlayer)) return;

                // Only split if this item was removed from TC recently
                if (!_fromCupboard.Remove(item.uid)) return;

                // Normal max outside cupboard
                int normalMax = Mathf.Clamp(GetStackSize(item.info), 1, int.MaxValue);
                if (item.amount <= normalMax) return;

                int excess = item.amount - normalMax;
                item.amount = normalMax;
                item.MarkDirty();

                _ssctcSplittingGuard = true;
                try
                {
                    while (excess > 0)
                    {
                        int chunk = Mathf.Min(excess, normalMax);
                        var extra = ItemManager.Create(item.info, chunk, item.skin);
                        if (!extra.MoveToContainer(container))
                        {
                            BasePlayer player = null; BaseEntity owner = null;
                            try { player = container.playerOwner; } catch { }
                            try { owner  = container.entityOwner; } catch { }
                            ...
                            try
                            {
                                ...
                                else if (owner != null) pos = owner.transform.position;
                            } catch { }
                            extra.Drop(pos, UnityEngine.Vector3.zero);
                        }
                        excess -= chunk;
                    }
                }
                finally { _ssctcSplittingGuard = false; }
            }
            catch { /* ignore */ }
        }
